...
async function cargarCambios() {
    ...
    try {
        ...
        if (!listaRes) {
            console.log('Respuesta vacía para lista_empresa en cargarCambios');
            return;
        }
        
        // Si no es una cadena, intentar convertirla
        if (typeof listaRes !== 'string') {
            listaRes = String(listaRes);
        }
        
        // Verificar si la respuesta es JSON válido antes de parsear
        if (listaRes.trim() === '' || listaRes.trim().startsWith('<') || listaRes.includes('<br') || listaRes.includes('Query Falló') || listaRes.includes('Successfully') || listaRes.trim() === 'No') {
            console.log('Respuesta no válida para lista_empresa en cargarCambios:', listaRes);
            return;
        }
        ...
        if (typeof listaRes === 'string') {
            try {
                lista = JSON.parse(listaRes);
            } catch (error) {
                console.error('Error parseando JSON en cargarCambios:', error, 'Respuesta:', listaRes);
                return;
            }
        } else {
            lista = listaRes; // jQuery ya parseó el JSON
        }
        ...
        for (const item of lista) {
            ...
            if (mesActual >= 12) {
                query = `SELECT CONCAT_WS( ' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada ) empleado, em.nombre_comercial empresa, e.fecha_inicio, CASE WHEN e.fecha_inicio <= DATE_FORMAT(NOW(), '%Y-12-01') THEN DATE_FORMAT( DATE_FORMAT(NOW(), '%Y-12-01'), '%Y-%m-%d') ELSE DATE_FORMAT(e.fecha_inicio, '%Y-%m-%d') END AS periodo_pago, DATE_FORMAT( NOW() + INTERVAL 1 YEAR, '%Y-11-30') AS AL, DATEDIFF( DATE_FORMAT( NOW() + INTERVAL 1 YEAR, '%Y-12-01'), CASE WHEN e.fecha_inicio <= DATE_FORMAT(NOW(), '%Y-12-01') THEN DATE_FORMAT( DATE_FORMAT(NOW(), '%Y-12-01'), '%Y-%m-%d') ELSE DATE_FORMAT(e.fecha_inicio, '%Y-%m-%d') END ) AS dias_pagar, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 12 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS diciembre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 1 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS enero, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 2 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS febrero, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 3 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS marzo, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 4 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS abril, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 5 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS mayo, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 6 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS junio, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 7 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS julio, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 8 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS agosto, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 9 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS septiembre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 10 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS octubre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 11 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS noviembre, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo, ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) AS bono, ( ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) / 2 ) AS primer_pago, ( ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) / 2 ) AS segundo_pago FROM pago_lote pl INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT(NOW(), '%Y-12-01') AND DATE_FORMAT( NOW() + INTERVAL 1 YEAR, '%Y-11-30') AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${item.id} GROUP BY e.id`;
            } else {
                query = `SELECT CONCAT_WS( ' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada ) empleado, em.nombre_comercial empresa, e.fecha_inicio, CASE WHEN e.fecha_inicio <= DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01') THEN DATE_FORMAT( DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01'), '%Y-%m-%d' ) ELSE DATE_FORMAT(e.fecha_inicio, '%Y-%m-%d') END AS periodo_pago, DATE_FORMAT(NOW(), '%Y-11-30') AS AL, DATEDIFF( DATE_FORMAT(NOW(), '%Y-12-01'), CASE WHEN e.fecha_inicio <= DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01') THEN DATE_FORMAT( DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01'), '%Y-%m-%d' ) ELSE DATE_FORMAT(e.fecha_inicio, '%Y-%m-%d') END ) AS dias_pagar, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 7 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS julio, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 8 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS agosto, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 9 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS septiembre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 10 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS octubre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 11 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS noviembre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 12 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS diciembre, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 1 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS enero, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 2 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS febrero, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 3 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS marzo, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 4 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS abril, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 5 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS mayo, SUM( CASE WHEN MONTH(pl.fecha_pago_lote) = 6 THEN pl.total_reporte_bono * ee.porcentaje / 100 ELSE 0 END ) AS junio, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo, ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) AS bono, ( ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) / 2 ) AS primer_pago, ( ( SUM(pl.total_reporte_bono) / 360 * 30 * ee.porcentaje / 100 ) / 2 ) AS segundo_pago FROM pago_lote pl INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01') AND DATE_FORMAT(NOW(), '%Y-11-30') AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${item.id} GROUP BY e.id`;
            }
            ...
            if (res && typeof res === 'string' && res.trim() != '') {
                ...
                if (res.trim().startsWith('<') || res.includes('<br') || res.includes('Query Falló') || res.includes('Successfully') || res.trim() === 'No') {
                    console.log('Respuesta no válida para cambios_personal:', res);
                ...
                if (typeof res === 'string') {
                    try {
                        ...
                        console.error('Error parseando JSON en cambios_personal:', error, 'Respuesta:', res);
                        continue; // Saltar a la siguiente iteración del bucle
                    }
                } else {
...
function excel_cambios_empresa(id_empresa) {
    ...
    return new Promise((resolve) => {
        ...
        resolve(window.location.href = './php/cambios_empresa_aguinaldo.php?query=' + query);
    });
}
...
async function dinamismo() {
    ...
    // Verificar si la respuesta es válida antes de procesar
    if (!listaRes) {
        console.log('Respuesta vacía para lista_empresa en dinamismo');
        return;
    }
    ...
    // Verificar si la respuesta es JSON válido antes de parsear
    if (listaRes.trim() === '' || listaRes.trim().startsWith('<') || listaRes.includes('<br') || listaRes.includes('Query Falló') || listaRes.includes('Successfully') || listaRes.trim() === 'No') {
        console.log('Respuesta no válida para lista_empresa en dinamismo:', listaRes);
        return;
    }
    ...
    if (typeof listaRes === 'string') {
        try {
            ...
            console.error('Error parseando JSON en dinamismo:', error, 'Respuesta:', listaRes);
            return;
        }
    } else {
        lista = listaRes; // jQuery ya parseó el JSON
    ...
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res == 'No') {
                    btn_excel_aguinaldo_empleados.style.display = 'none';
                } else {
                    btn_excel_aguinaldo_empleados.style.display = '';
                }
                
                // Verificar si la respuesta es válida antes de procesar
                if (!res) {
                    console.log('Respuesta vacía para lista_aguinaldo_anual');
                    resolve();
                    return;
                }
                ...
                // Verificar si la respuesta es JSON válido antes de parsear
                if (res.trim().startsWith('<') || res.includes('<br') || res.includes('Query Falló') || res.includes('Successfully') || res.trim() === 'No') {
                    console.log('Respuesta no válida para lista_aguinaldo_anual:', res);
                    resolve();
                    return;
                }
                ...
                if (typeof res === 'string') {
                    try {
                        ...
                        console.error('Error parseando JSON en lista_aguinaldo_anual:', error, 'Respuesta:', res);
                        resolve();
                        return;
                    }
                } else {
                ...
                lista.forEach(item => {
                    ...
                    <tr role="row">
                        ...
                        <td>${item.bono}</td>
                        <td>${item.primer_pago}</td>
                        <td>${item.segundo_pago}</td>
                        <td>
                        </td>
                    </tr>
                    `;
...
function excel_aguinaldo_empleados() {
    const mesActual = fecha.getMonth() + 1;
    var anio = 2000;
    ...
    return new Promise((resolve) => {
        ...
        resolve(window.location.href = './php/aguinaldo_empleados.php?anio=' + anio + '&id_empresa=' + id_empresa);
    });
}
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (res) {
                // Verificar si la respuesta es válida antes de procesar
                if (!res) {
                    console.log('Respuesta vacía para lista_empresa en cargarEmpresasDeudoras');
                    resolve();
                    return;
                }
                ...
                // Verificar si la respuesta es JSON válido antes de parsear
                if (res.trim() === '' || res.trim().startsWith('<') || res.includes('<br') || res.includes('Query Falló') || res.includes('Successfully') || res.trim() === 'No') {
                    console.log('Respuesta no válida para lista_empresa en cargarEmpresasDeudoras:', res);
                    resolve();
                    return;
                }
                ...
                if (typeof res === 'string') {
                    try {
                        listaEmpresas = JSON.parse(res);
                    } catch (error) {
                        console.error('Error parseando JSON en cargarEmpresasDeudoras:', error, 'Respuesta:', res);
                        resolve();
                        return;
                    }
                } else {
                    listaEmpresas = res; // jQuery ya parseó el JSON
                }
                let loadedPromises = [];
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (resp) {
                // Verificar si la respuesta es válida antes de procesar
                if (!resp) {
                    console.log('Respuesta vacía para lista_empresa_filtro en cargarEmpresaDeudora');
                    resolve();
                    return;
                }
                ...
                // Verificar si la respuesta es JSON válido antes de parsear
                if (resp.trim() === '' || resp.trim().startsWith('<') || resp.includes('<br') || resp.includes('Query Falló') || resp.includes('Successfully') || resp.trim() === 'No') {
                    console.log('Respuesta no válida para lista_empresa_filtro en cargarEmpresaDeudora:', resp);
                    resolve();
                    return;
                }
                ...
                if (typeof resp === 'string') {
                    try {
                        listaDeudas = JSON.parse(resp);
                    } catch (error) {
                        console.error('Error parseando JSON en cargarEmpresaDeudora:', error, 'Respuesta:', resp);
                        resolve();
                        return;
                    }
                } else {
                    listaDeudas = resp; // jQuery ya parseó el JSON
                }
                let loadedPromises = [];
...
async function obtenerIntercompanyData(deuda, empresa) {
    ...
    return new Promise((resolve, reject) => {
        if (mesActual >= 12) {
            template = `SELECT CONCAT_WS( ' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada ) empleado, eD.nombre_comercial empresa_deudora, ( SELECT eA.nombre_comercial FROM empresa_empleado ee2 INNER JOIN empresa eA ON eA.id = ee2.id_empresa WHERE ee2.id_empleado = e.id AND ee2.principal = 1 ) empresa_acreedora, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo FROM pago_lote pl INNER JOIN empresa eD ON eD.id = pl.id_empresa INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT(NOW(), '%Y-12-01') AND DATE_FORMAT( NOW() + INTERVAL 1 YEAR, '%Y-11-30') AND pl.id_empresa !=( SELECT ee2.id_empresa FROM empresa_empleado ee2 WHERE ee2.id_empleado = e.id AND ee2.principal = 1 AND ee2.id_empresa = ${ empresa.id } ) AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${ deuda.id } GROUP BY e.id`; // Tu consulta SQL
        } else {
            template = `SELECT CONCAT_WS(' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada) empleado, eD.nombre_comercial empresa_deudora, ( SELECT eA.nombre_comercial FROM empresa_empleado ee2 INNER JOIN empresa eA ON eA.id = ee2.id_empresa WHERE ee2.id_empleado = e.id AND ee2.principal = 1 ) empresa_acreedora, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo FROM pago_lote pl INNER JOIN empresa eD ON eD.id = pl.id_empresa INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01') AND DATE_FORMAT(NOW(), '%Y-11-30') AND pl.id_empresa !=( SELECT ee2.id_empresa FROM empresa_empleado ee2 WHERE ee2.id_empleado = e.id AND ee2.principal = 1 AND ee2.id_empresa = ${empresa.id} ) AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${deuda.id} GROUP BY e.id`; // Tu consulta SQL
        }
        $.ajax({
            url: 'php/servidor.php',
            type: 'GET',
            ...
            error: function (error) {
                console.error('Error en obtenerIntercompanyData:', error);
                resolve('');
            }
        });
    });
...
function generarTablaIntercompany(intercompanyData, deuda, empresa) {
    ...
    // Verificar si la respuesta es válida antes de procesar
    if (!intercompanyData) {
        console.log('Respuesta vacía para intercompanyData en generarTablaIntercompany');
        return tabla_intercompany; // Retornar la tabla sin datos
    }
    
    // Si no es una cadena, intentar convertirla
    if (typeof intercompanyData !== 'string') {
        intercompanyData = String(intercompanyData);
    }
    
    // Verificar si la respuesta es JSON válido antes de parsear
    if (intercompanyData.trim() === '' || intercompanyData.trim().startsWith('<') || intercompanyData.includes('<br') || intercompanyData.includes('Query Falló') || intercompanyData.includes('Successfully') || intercompanyData.trim() === 'No') {
        console.log('Respuesta no válida para intercompanyData en generarTablaIntercompany:', intercompanyData);
        return tabla_intercompany; // Retornar la tabla sin datos
    }
    
    let lista_intercompanyData;
    if (typeof intercompanyData === 'string') {
        try {
            lista_intercompanyData = JSON.parse(intercompanyData);
        } catch (error) {
            console.error('Error parseando JSON en generarTablaIntercompany:', error, 'Respuesta:', intercompanyData);
            return tabla_intercompany; // Retornar la tabla sin datos
        }
    } else {
        lista_intercompanyData = intercompanyData; // jQuery ya parseó el JSON
    }
...
function excel_facturacion(deuda, empresa) {
    ...
    return new Promise((resolve) => {
        if (mesActual >= 12) {
            template = `SELECT CONCAT_WS( ' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada ) empleado, eD.nombre_comercial empresa_deudora, ( SELECT eA.nombre_comercial FROM empresa_empleado ee2 INNER JOIN empresa eA ON eA.id = ee2.id_empresa WHERE ee2.id_empleado = e.id AND ee2.principal = 1 ) empresa_acreedora, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo FROM pago_lote pl INNER JOIN empresa eD ON eD.id = pl.id_empresa INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT(NOW(), '%Y-12-01') AND DATE_FORMAT( NOW() + INTERVAL 1 YEAR, '%Y-11-30') AND pl.id_empresa !=( SELECT ee2.id_empresa FROM empresa_empleado ee2 WHERE ee2.id_empleado = e.id AND ee2.principal = 1 AND ee2.id_empresa = ${empresa} ) AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${deuda} GROUP BY e.id`; // Tu consulta SQL
        } else {
            template = `SELECT CONCAT_WS(' ', e.primer_nombre, e.segundo_nombre, e.otro_nombre, e.primer_apellido, e.segundo_apellido, e.apellido_casada) empleado, eD.nombre_comercial empresa_deudora, ( SELECT eA.nombre_comercial FROM empresa_empleado ee2 INNER JOIN empresa eA ON eA.id = ee2.id_empresa WHERE ee2.id_empleado = e.id AND ee2.principal = 1 ) empresa_acreedora, SUM( pl.total_reporte_bono * ee.porcentaje / 100 ) AS total_salario_periodo FROM pago_lote pl INNER JOIN empresa eD ON eD.id = pl.id_empresa INNER JOIN empleado e ON pl.id_empleado = e.id RIGHT JOIN empresa_empleado ee ON e.id = ee.id_empleado INNER JOIN empresa em ON ee.id_empresa = em.id WHERE pl.fecha_pago_lote BETWEEN DATE_FORMAT( NOW() - INTERVAL 1 YEAR, '%Y-12-01') AND DATE_FORMAT(NOW(), '%Y-11-30') AND pl.id_empresa !=( SELECT ee2.id_empresa FROM empresa_empleado ee2 WHERE ee2.id_empleado = e.id AND ee2.principal = 1 AND ee2.id_empresa = ${empresa} ) AND e.id IN( SELECT DISTINCT a.id_empleado FROM pago_lote a INNER JOIN pago_lote b ON a.id_empleado = b.id_empleado AND a.fecha_pago_lote < b.fecha_pago_lote WHERE a.id_empresa <> b.id_empresa GROUP BY a.id_empleado ) AND pl.id_empresa = ${deuda} GROUP BY e.id`; // Tu consulta SQL
        }
        resolve(window.location.href = './php/facturacion_aguinaldo.php?query=' + template);
    });
}
...
function obtenerTotalIntercompany(deuda, empresa, callback) {
    ...
    $.ajax({
        ...
        error: function (error) {
            console.error('Error en obtenerTotalIntercompany:', error);
            callback(error, null);
        }
    });
}
