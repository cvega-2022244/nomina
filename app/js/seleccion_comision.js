        ...
        $.ajax({
            ...
            success: function (res) {
                try {
                    ...
                    if (res.includes('<br />') || res.includes('<b>') || res.includes('<!DOCTYPE')) {
                        console.error('El servidor devolvió HTML en lugar de JSON:', res);
                        Swal.fire({
                            icon: 'error',
                            title: 'Error del Servidor',
                            text: 'El servidor devolvió una respuesta HTML. Revisa la consola para más detalles.'
                        });
                        return;
                    }
                ...
                if (lista != 0) {
                    lista.forEach(lista => {
                        ...
                        <tr role="row">
                            ...
                            value="" onclick="confirmar_comision(${lista.id})"`
                        ...
                        template += `
                            <td class="text-center">
                                <div class="action-btns">
                                    ...
                                    <a onclick="advertencia_anular_comision(${lista.id})"class="action-btn btn-delete bs-tooltip"
                    ...
                    try {
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: 'Ninguna Comision Autorizada',
                            text: 'Por el momento no hay ningun comisiones autorizadas'
                        });
                    } catch (error) {
                        console.log(error);
                    } finally {
                    ...
                    Swal.fire({
                        ...
                        text: 'Error al procesar la respuesta del servidor. Revisa la consola para más detalles.'
                    });
                    resolve('false');
                }
            }
...
function detalle(id, id_empleado) {
    sessionStorage.setItem("id_comision", id);
    sessionStorage.setItem("id_empleado", id_empleado);
    window.location.href = './detalle_comision.html';
}

function advertencia_anular_comision(id) {
    Swal.fire({
        icon: 'question',
        title: '¿Está seguro de anular la comision?',
...
function anular_comision(id) {
    return new Promise((resolve) => {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.includes('Query Falló')) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error Al Anular Comision',
                        text: 'Por favor, comunicate con sistemas'
                    });
                    console.log(res);
                } else {
                    try {
                        console.log('Comision Anulado');
                    } catch (error) {
                        console.log(error);
                    } finally {
                        resolve(res);
        ...
        Swal.fire({
            icon: 'success',
            title: 'Comision Anulada',
            text: 'La comision ha sido anulado exitosamente'
        }).then(() => {
            listado_comisiones();
        });
    })
            ...
            $.ajax({
                ...
                success: function (respuesta) {
                    if (respuesta.includes('Query Falló')) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error Al Desaprobar Comision',
