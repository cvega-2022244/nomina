        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (lista != 0) {
                    lista.forEach(lista => {
                        ...
                        if (lista.estado == 'Solicitado') {
                            template += `
                                ...
                                `;
                        } else if (lista.estado == 'Autorizado' || lista.estado == 'Aprobado RH') {
                            template += `
                                    <td><div class="badge badge-primary badge-pill">${lista.estado}</div></td>
                                `;
                    ...
                    // Destruir DataTable solo si ya existe
        ...
        if (chx.checked == true) {
            $.ajax({
                ...
                success: function (respuesta) {
                    Swal.close(); // Cerrar modal inmediatamente
                    if (respuesta.includes('Query Falló')) {
                        chx.checked = false; // Desmarcar si falla
                        Swal.fire({
                            icon: 'error',
                        ...
                        // Mantener checkbox marcado y mostrar mensaje breve
                        chx.checked = true;
                        console.log('✅ Bono ' + id + ' confirmado exitosamente');
            ...
            $.ajax({
                ...
                success: function (respuesta) {
                    Swal.close(); // Cerrar modal inmediatamente
                    if (respuesta.includes('Query Falló')) {
                        chx.checked = true; // Volver a marcar si falla
                        Swal.fire({
                            icon: 'error',
                        ...
                        // Mantener checkbox desmarcado y mostrar mensaje breve
                        chx.checked = false;
                        console.log('ℹ️ Bono ' + id + ' desmarcado');
