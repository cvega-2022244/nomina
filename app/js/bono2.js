    ...
    try {
        const listaRes = await $.ajax({
            ...
            error: function(xhr, status, error) {
                console.error('Error en lista_empresa:', error);
            }
        });

        // Verificar si la respuesta es JSON válido antes de parsear
        if (!listaRes || listaRes.trim() === '' || listaRes.trim().startsWith('<') || listaRes.includes('<br') || listaRes.includes('Query Falló') || listaRes.includes('Successfully') || listaRes.trim() === 'No') {
        ...
        if (typeof listaRes === 'string') {
            try {
                ...
                console.error('Error parseando JSON en cargarCambios lista_empresa:', error, 'Respuesta:', listaRes);
                return;
            }
        } else {
        ...
        for (const item of lista) {
            ...
            try {
                ...
                console.error('Error en cambios_personal:', error);
...
async function dinamismo() {
    const listaRes = await $.ajax({
        ...
        dataType: 'text', // Evitar que jQuery intente parsear como JSON
        error: function(xhr, status, error) {
            console.error('Error en dinamismo lista_empresa:', error);
        }
    });

    // Verificar si la respuesta es válida antes de procesar
    if (!listaRes) {
        console.log('Respuesta vacía para dinamismo lista_empresa');
        return;
    }
    ...
    if (listaRes.trim() === '' || listaRes.trim().startsWith('<') || listaRes.includes('<br') || listaRes.includes('Query Falló') || listaRes.includes('Successfully') || listaRes.trim() === 'No') {
        console.log('Respuesta no válida para dinamismo lista_empresa:', listaRes);
        return;
    }
    ...
    if (typeof listaRes === 'string') {
        try {
            ...
            console.error('Error parseando JSON en dinamismo lista_empresa:', error, 'Respuesta:', listaRes);
            return;
        }
    } else {
        lista = listaRes; // jQuery ya parseó el JSON
    ...
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // Verificar si la respuesta es válida antes de procesar
                if (!res) {
                    console.log('Respuesta vacía para lista_bono_anual');
                    resolve();
                    return;
                }
                ...
                // Verificar si la respuesta es JSON válido antes de parsear
                if (res.trim().startsWith('<') || res.includes('<br') || res.includes('Query Falló') || res.includes('Successfully') || res.trim() === 'No') {
                    console.log('Respuesta no válida para lista_bono_anual:', res);
                    resolve();
                    return;
                }
                ...
                if (typeof res === 'string') {
                    try {
                        ...
                        console.error('Error parseando JSON en lista_bono_anual:', error, 'Respuesta:', res);
                        resolve();
                        return;
                    }
                } else {
...
function cargarEmpresasDeudoras() {
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (res) {
                // Verificar si la respuesta es JSON válido antes de parsear
                if (!res || res.trim() === '' || res.trim().startsWith('<') || res.includes('<br') || res.includes('Query Falló') || res.includes('Successfully') || res.trim() === 'No') {
                    console.log('Respuesta no válida para lista_empresa:', res);
                    resolve();
                    return;
                }
                ...
                if (typeof res === 'string') {
                    try {
                        ...
                        console.error('Error parseando JSON en lista_empresa:', error, 'Respuesta:', res);
                        resolve();
                        return;
                    }
                } else {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (resp) {
                // Verificar si la respuesta es JSON válido antes de parsear
                if (!resp || resp.trim() === '' || resp.trim().startsWith('<') || resp.includes('<br') || resp.includes('Query Falló') || resp.includes('Successfully') || resp.trim() === 'No') {
                    console.log('Respuesta no válida para lista_empresa_filtro:', resp);
                    resolve();
                    return;
                }
                ...
                if (typeof resp === 'string') {
                    try {
                        ...
                        console.error('Error parseando JSON en lista_empresa_filtro:', error, 'Respuesta:', resp);
                        resolve();
                        return;
                    }
                } else {
...
async function obtenerIntercompanyData(deuda, empresa) {
    ...
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                console.error('Error en intercompany_bonos:', error);
                resolve('');
            }
        });
    });
...
function generarTablaIntercompany(intercompanyData, deuda, empresa) {
    ...
    // Verificar si la respuesta es válida antes de procesar
    if (!intercompanyData) {
        console.log('Respuesta vacía para intercompanyData');
    ...
    if (intercompanyData.trim() === '' || intercompanyData.trim().startsWith('<') || intercompanyData.includes('<br') || intercompanyData.includes('Query Falló') || intercompanyData.includes('Successfully') || intercompanyData.trim() === 'No') {
        console.log('Respuesta no válida para intercompanyData:', intercompanyData);
        return tabla_intercompany; // Retornar la tabla sin datos
    }
    ...
    if (typeof intercompanyData === 'string') {
        try {
            ...
            console.error('Error parseando JSON en intercompanyData:', error, 'Respuesta:', intercompanyData);
            return tabla_intercompany; // Retornar la tabla sin datos
        }
    } else {
