    ...
    if (isset($_GET["quest"])) {
        ...
        if ($_GET["quest"] == 'lista_comisiones_rechazadas') {
            ...
            if ($user_id && $user_role == 'empleado') {
                // Si es empleado, mostrar solo sus solicitudes rechazadas
                $sql = "SELECT c.id, c.id_empleado, CONCAT(e.primer_nombre, ' ', e.primer_apellido) as empleado, COALESCE(d.nombre, 'Sin Departamento') as departamento, COALESCE(u.nombre, 'Sin Usuario') as solicitante, c.fecha_generado, c.monto, 'Rechazada' as estado, c.seleccionado FROM comision c LEFT JOIN empleado e ON c.id_empleado = e.id LEFT JOIN departamento d ON e.departamento_laboral = d.id LEFT JOIN usuario u ON c.id_solicitante = u.id WHERE c.id_estado = 3 AND c.id_solicitante = " . $user_id . " ORDER BY c.fecha_generado DESC";
            } else if ($user_id && ($user_role == 'jefe' || $user_role == 'gerente')) {
                // Si es jefe o gerente, mostrar solicitudes rechazadas de sus empleados (ambos tienen el mismo nivel)
                $sql = "SELECT c.id, c.id_empleado, CONCAT(e.primer_nombre, ' ', e.primer_apellido) as empleado, COALESCE(d.nombre, 'Sin Departamento') as departamento, COALESCE(u.nombre, 'Sin Usuario') as solicitante, c.fecha_generado, c.monto, 'Rechazada' as estado, c.seleccionado 
                        FROM comision c 
                        ...
                        INNER JOIN jerarquia_organizacional j ON e.id = j.id_empleado 
                        WHERE c.id_estado = 3 AND (j.id_jefe = " . $user_id . " OR j.id_gerente = " . $user_id . ") AND j.activo = 1
                        ORDER BY c.fecha_generado DESC";
            } else if ($user_role == 'rh') {
                // Si es RH, mostrar todas las solicitudes rechazadas
                $sql = "SELECT c.id, c.id_empleado, CONCAT(e.primer_nombre, ' ', e.primer_apellido) as empleado, COALESCE(d.nombre, 'Sin Departamento') as departamento, COALESCE(u.nombre, 'Sin Usuario') as solicitante, c.fecha_generado, c.monto, 'Rechazada' as estado, c.seleccionado FROM comision c LEFT JOIN empleado e ON c.id_empleado = e.id LEFT JOIN departamento d ON e.departamento_laboral = d.id LEFT JOIN usuario u ON c.id_solicitante = u.id WHERE c.id_estado = 3 ORDER BY c.fecha_generado DESC";
            } else {
                // Por defecto, mostrar todas las solicitudes rechazadas
                $sql = "SELECT c.id, c.id_empleado, CONCAT(e.primer_nombre, ' ', e.primer_apellido) as empleado, COALESCE(d.nombre, 'Sin Departamento') as departamento, COALESCE(u.nombre, 'Sin Usuario') as solicitante, c.fecha_generado, c.monto, 'Rechazada' as estado, c.seleccionado FROM comision c LEFT JOIN empleado e ON c.id_empleado = e.id LEFT JOIN departamento d ON e.departamento_laboral = d.id LEFT JOIN usuario u ON c.id_solicitante = u.id WHERE c.id_estado = 3 ORDER BY c.fecha_generado DESC";
            }
        ...
        if ($_GET["quest"] == 'lista_comisiones_autorizados') {
            ...
            if ($user_id && $user_role == 'empleado') {
                // Si es empleado, mostrar solo sus solicitudes autorizadas
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE b.id_solicitante = " . $user_id . " AND eb.id IN (6, 8) GROUP BY b.id ORDER BY b.id DESC";
            } else if ($user_id && ($user_role == 'jefe' || $user_role == 'gerente')) {
                // Si es jefe o gerente, mostrar solicitudes autorizadas de sus empleados (ambos tienen el mismo nivel)
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado 
                        FROM comision b 
                        INNER JOIN empleado emp ON b.id_empleado = emp.id 
                        INNER JOIN departamento d ON emp.departamento_laboral = d.id 
                        INNER JOIN usuario u ON b.id_solicitante = u.id 
                        INNER JOIN estado_bono eb ON b.id_estado = eb.id 
                        INNER JOIN jerarquia_organizacional j ON emp.id = j.id_empleado 
                        WHERE (j.id_jefe = " . $user_id . " OR j.id_gerente = " . $user_id . ") AND eb.id IN (6, 8) AND j.activo = 1
                        GROUP BY b.id ORDER BY b.id DESC";
            } else if ($user_role == 'rh') {
                // Si es RH, mostrar todas las solicitudes autorizadas
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE eb.id IN (8) GROUP BY b.id ORDER BY b.id DESC";
            } else {
                // Por defecto, mostrar todas las solicitudes autorizadas
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE eb.id IN (8) GROUP BY b.id ORDER BY b.id DESC";
            }
        ...
        if ($_GET["quest"] == 'lista_comisiones_pendientes') {
            ...
            if ($user_id && $user_role == 'empleado') {
                // Si es empleado, mostrar solo sus solicitudes
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE b.id_solicitante = " . $user_id . " GROUP BY b.id ORDER BY b.id DESC";
            } else if ($user_id && ($user_role == 'jefe' || $user_role == 'gerente')) {
                // Si es jefe o gerente, mostrar solicitudes de sus empleados (ambos tienen el mismo nivel)
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado 
                        FROM comision b 
                        INNER JOIN empleado emp ON b.id_empleado = emp.id 
                        INNER JOIN departamento d ON emp.departamento_laboral = d.id 
                        ...
                        INNER JOIN estado_bono eb ON b.id_estado = eb.id 
                        INNER JOIN jerarquia_organizacional j ON emp.id = j.id_empleado 
                        WHERE (j.id_jefe = " . $user_id . " OR j.id_gerente = " . $user_id . ") AND eb.id IN (5, 7) AND j.activo = 1
                        GROUP BY b.id ORDER BY b.id DESC";
            } else if ($user_role == 'rh') {
                // Si es RH, mostrar todas las solicitudes pendientes
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE eb.id IN (5, 7) GROUP BY b.id ORDER BY b.id DESC";
            } else {
                // Por defecto, mostrar todas las solicitudes pendientes
                $sql = "SELECT b.id, d.nombre departamento, CONCAT( emp.primer_nombre, ' ', emp.primer_apellido ) empleado, emp.id id_empleado, u.nombre solicitante, DATE(b.fecha_generado) fecha_generado, b.monto, eb.nombre estado, b.seleccionado FROM comision b INNER JOIN empleado emp ON b.id_empleado = emp.id INNER JOIN departamento d ON emp.departamento_laboral = d.id INNER JOIN usuario u ON b.id_solicitante = u.id INNER JOIN estado_bono eb ON b.id_estado = eb.id WHERE eb.id IN (5, 7) GROUP BY b.id ORDER BY b.id DESC";
            }
        ...
        if ($_GET["quest"] == 'detalle_comision') {
            $sql = "SELECT b.id, COALESCE(e.nombre_comercial, 'Sin Empresa') as empresa, b.fecha_trabajado, DATE(b.fecha_generado) as fecha_generado, b.horas, b.tipo_jornada, b.monto, b.tarea, COALESCE(u.nombre, 'Sin Usuario') as usuario, COALESCE(eb.nombre, 'Sin Estado') as estado, '' as observacion FROM comision b LEFT JOIN empresa e ON b.empresa_trabajo = e.id LEFT JOIN usuario u ON b.id_solicitante = u.id LEFT JOIN estado_bono eb ON b.id_estado = eb.id WHERE b.id = " . $_GET["id"];
            $result = mysqli_query($con, $sql);
    ...
    if (isset($_POST['quest'])) {
        ...
        if ($_POST["quest"] == 'aprobar_comision') {
            ...
            if ($rol_aprobador == 'jefe' || $rol_aprobador == 'gerente') {
                $nuevo_estado = 7; // Pendiente RH (jefes y gerentes van directo a RH)
            } else if ($rol_aprobador == 'rh') {
                $nuevo_estado = 8; // Aprobado RH
            }
            ...
            // Solo RH puede actualizar la bonificación en pago_lote
            if ($rol_aprobador == 'rh') {
        ...
        if ($_POST["quest"] == 'crear_comision') {
            ...
            $sql = "INSERT INTO comision (empresa_trabajo, area_trabajo, puesto_trabajo, fecha_trabajado, fecha_generado, horas, tipo_jornada, monto, tarea, id_empleado, id_solicitante, id_estado, observacion_gerencia, seleccionado) VALUES (" . 
                   ...
                   $_POST['id_empleado'] . ", " . 
                   $_POST['id_solicitante'] . ", " . $estado_inicial . ", '" . 
                   mysqli_real_escape_string($con, $_POST['observacion_gerencia']) . "', 0)";
        ...
        // Obtener jefe de un empleado
        if ($_GET["quest"] == 'obtener_jefe_empleado') {
            ...
            $sql = "SELECT j.id_jefe, j.id_gerente, u.nombre as nombre_jefe, g.nombre as nombre_gerente 
                    FROM jerarquia_organizacional j 
                    LEFT JOIN usuario u ON j.id_jefe = u.id 
                    LEFT JOIN usuario g ON j.id_gerente = g.id 
                    WHERE j.id_empleado = " . intval($id_empleado) . " AND j.activo = 1";
            ...
            if ($data) {
                ...
                echo json_encode(['error' => 'No se encontró jerarquía para este empleado']);
            }
        }

        // Obtener empleados bajo supervisión de un jefe/gerente
        if ($_GET["quest"] == 'obtener_empleados_supervisados') {
            $id_supervisor = $_GET['id_supervisor'] ?? null;
            $tipo_supervisor = $_GET['tipo_supervisor'] ?? 'jefe'; // 'jefe' o 'gerente'
            
            if (!$id_supervisor) {
                echo json_encode(['error' => 'ID de supervisor requerido']);
                exit;
            }
            
            $campo = ($tipo_supervisor == 'gerente') ? 'id_gerente' : 'id_jefe';
            
            $sql = "SELECT j.id_empleado, u.nombre as nombre_empleado, d.nombre as departamento 
                    FROM jerarquia_organizacional j 
                    INNER JOIN usuario u ON j.id_empleado = u.id 
                    LEFT JOIN departamento d ON j.id_departamento = d.id 
                    WHERE j." . $campo . " = " . intval($id_supervisor) . " AND j.activo = 1";
        ...
        // Lista de usuarios para gestión de jerarquía
        ...
        // Lista de jerarquías
        if ($_GET["quest"] == 'lista_jerarquias') {
            $sql = "SELECT j.id, 
                           CONCAT(u1.primer_nombre, ' ', u1.primer_apellido) as empleado,
                           CONCAT(u2.primer_nombre, ' ', u2.primer_apellido) as jefe,
                           CONCAT(u3.primer_nombre, ' ', u3.primer_apellido) as gerente,
                           d.nombre as departamento,
                           j.activo
                    FROM jerarquia_organizacional j
                    LEFT JOIN usuario u1 ON j.id_empleado = u1.id
                    LEFT JOIN usuario u2 ON j.id_jefe = u2.id
                    LEFT JOIN usuario u3 ON j.id_gerente = u3.id
                    LEFT JOIN departamento d ON j.id_departamento = d.id
                    ORDER BY j.id DESC";
        ...
        if ($_GET["quest"] == 'estadisticas_usuario') {
            $user_id = $_GET['user_id'] ?? null;
            ...
            $sql_pendientes = "SELECT COUNT(*) as count FROM comision WHERE id_solicitante = " . intval($user_id) . " AND id_estado IN (5, 7)";
            $sql_aprobadas = "SELECT COUNT(*) as count FROM comision WHERE id_solicitante = " . intval($user_id) . " AND id_estado IN (8)";
                $sql_rechazadas = "SELECT COUNT(*) as count FROM comision WHERE id_solicitante = " . intval($user_id) . " AND id_estado = 3";
                $sql_total = "SELECT COALESCE(SUM(monto), 0) as total FROM comision WHERE id_solicitante = " . intval($user_id);
                
                $result_pendientes = mysqli_query($con, $sql_pendientes);
                $result_aprobadas = mysqli_query($con, $sql_aprobadas);
                $result_rechazadas = mysqli_query($con, $sql_rechazadas);
                $result_total = mysqli_query($con, $sql_total);
                
                if ($result_pendientes) {
                    $row = mysqli_fetch_array($result_pendientes);
                    $pendientes = $row ? $row['count'] : 0;
                }
                
                if ($result_aprobadas) {
                    $row = mysqli_fetch_array($result_aprobadas);
                    $aprobadas = $row ? $row['count'] : 0;
                }
                
                if ($result_rechazadas) {
                    $row = mysqli_fetch_array($result_rechazadas);
                    $rechazadas = $row ? $row['count'] : 0;
                }
                
                if ($result_total) {
                    $row = mysqli_fetch_array($result_total);
                    $total_monto = $row ? $row['total'] : 0;
                }
                
            } catch (Exception $e) {
                // En caso de error, usar valores por defecto
                error_log("Error en estadisticas_usuario: " . $e->getMessage());
            }
            
            echo json_encode([
                'pendientes' => intval($pendientes),
                'aprobadas' => intval($aprobadas),
                'rechazadas' => intval($rechazadas),
                'total_monto' => floatval($total_monto)
            ]);
        }
        ...
        // Guardar jerarquía
        if ($_POST["quest"] == 'guardar_jerarquia') {
            $id_empleado = $_POST['id_empleado'] ?? null;
            $id_jefe = $_POST['id_jefe'] ?? null;
            $id_gerente = $_POST['id_gerente'] ?? null;
            $id_departamento = $_POST['id_departamento'] ?? null;
            
            if (!$id_empleado || !$id_departamento) {
                echo json_encode(['error' => 'Empleado y departamento son requeridos']);
                exit;
            }
            
            // Verificar si ya existe una jerarquía para este empleado
            $sql_check = "SELECT id FROM jerarquia_organizacional WHERE id_empleado = " . intval($id_empleado);
            $result_check = mysqli_query($con, $sql_check);
            
            if (mysqli_num_rows($result_check) > 0) {
                // Actualizar jerarquía existente
                $sql = "UPDATE jerarquia_organizacional SET 
                        id_jefe = " . ($id_jefe ? intval($id_jefe) : 'NULL') . ",
                        id_gerente = " . ($id_gerente ? intval($id_gerente) : 'NULL') . ",
                        id_departamento = " . intval($id_departamento) . ",
                        activo = 1
                        WHERE id_empleado = " . intval($id_empleado);
            } else {
                // Crear nueva jerarquía
                $sql = "INSERT INTO jerarquia_organizacional (id_empleado, id_jefe, id_gerente, id_departamento, activo) 
                        VALUES (" . intval($id_empleado) . ", " . 
                        ($id_jefe ? intval($id_jefe) : 'NULL') . ", " . 
                        ($id_gerente ? intval($id_gerente) : 'NULL') . ", " . 
                        intval($id_departamento) . ", 1)";
            }
        ...
        // Eliminar jerarquía
        if ($_POST["quest"] == 'eliminar_jerarquia') {
            $jerarquia_id = $_POST['jerarquia_id'] ?? null;
            
            if (!$jerarquia_id) {
                echo json_encode(['error' => 'ID de jerarquía requerido']);
                exit;
            }
            
            $sql = "UPDATE jerarquia_organizacional SET activo = 0 WHERE id = " . intval($jerarquia_id);
            $result = mysqli_query($con, $sql);
