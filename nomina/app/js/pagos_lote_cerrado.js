                            ...
                            template += `
                                    <td class="text-center">
                                        <div class="action-btns">
                                            <a onclick="detalle(${lista.id_lote}, ${lista.id_empleado})" class="action-btn btn-view bs-tooltip me-2"
...
function listado_pagos_centro_costo(id_centro) {
    return new Promise((resolve) => {
        $.ajax({
            ...
            type: 'GET',
            data: {
                quest: 'listado_pagos_centro_historial',
                id_lote: id_lote_detalle,
                id_empresa: id_empresa_nomina,
                id_centro
            },
            success: function (res) {
                if (res.includes('Query Falló')) {
                    ...
                    try {
                        ...
                        let template = '';
                        lista.forEach(lista => {
                            template += `<tr><td>${lista.correlativo}</td>
                                    <td>${lista.empleado}</td>
                                    <td>${lista.nombre_comercial}</td>
                                    <td>${lista.centro_costo}</td>
                                    <td>${lista.departamento}</td>
...
function listado_pagos_departamento(id_departamento) {
    return new Promise((resolve) => {
        $.ajax({
            ...
            type: 'GET',
            data: {
                quest: 'listado_pagos_departamento_historial',
                id_lote: id_lote_detalle,
                id_empresa: id_empresa_nomina,
                id_departamento
            },
            ...
            success: function (res) {
                if (res.includes('Query Falló')) {
                    ...
                    try {
                        ...
                        let template = '';
                        lista.forEach(lista => {
                            template += `<td>${lista.tipo_egreso}</td>
