    ...
    try {
      $.ajax({
        ...
        success: function (resp) {
          ...
          selectBox = new vanillaSelectBox("#jornada", {
            keepInlineStyles: true,
            maxHeight: 678,
            minWidth: 200,
            search: true,
...
function obtener_edad_empleado() {
  ...
  var diaDef = 1000 * 60 * 60 * 24;
  var diferencia_dias = Math.ceil(Math.abs(fecha_actual - nacimiento) / diaDef);
  var diferencia_años = Math.floor(diferencia_dias / 365.25);
  var input_edad = document.getElementById("edad_empleado");
  input_edad.value = diferencia_años;
}
...
async function validar_empleado_repetido() {
  ...
  if (dpi.value == '') {
    ...
    } else if (resp.includes('No hay datos') || lista[0].cantidad == '0') {
      ingresar_empleado();
    } else {
      Swal.fire({
                        ...
                        return new Promise((resolve) => {
                          ...
                          return new Promise((resolve) => {
                            if (empresas.length >= 1) {
                              ingresar_empresa_anterior_db(resolve);
                            } else {
                              resolve("success");
                            }
                          }).then(() => {
...
function ingresar_empresa_empleado_db() {
  return new Promise((resolve) => {
    try {
      console.log(empresas_intercompany);
      empresas_intercompany.forEach((empresa) => {
        $.ajax({
          url: "php/servidor.php",
          type: "POST",
          data: {
            quest: "ingresar_empresa_empleado_db",
